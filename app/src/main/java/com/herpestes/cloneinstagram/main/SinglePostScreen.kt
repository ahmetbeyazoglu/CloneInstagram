...
fun SinglePostDisplay(navController: NavController, vm: IgViewModel, post: PostData) {
    ...
    Box(
        ...
        Row(modifier = Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
            ...
            if (userData?.userId == post.userId) {
                ...
                Text(text = "Following", color = Color.Gray, modifier = Modifier.clickable { vm.onFollowClick(post.userId!!) })
            }else{
    ...
    Row(modifier = Modifier.padding(8.dp), verticalAlignment = Alignment.CenterVertically) {
        Image(
            painter = painterResource(id = com.herpestes.cloneinstagram.R.drawable.ic_like),
            contentDescription = null,
